CREATE SCHEMA IF NOT EXISTS `__PROJECT__.retail_star`
OPTIONS(location="europe-west9", description="Retail star schema dataset for BI demos");

CREATE OR REPLACE TABLE `__PROJECT__.retail_star.dim_product` (
  product_key INT64,
  product_id STRING,
  product_name STRING,
  category STRING,
...
INSERT INTO `__PROJECT__.retail_star.dim_product` VALUES
...
CREATE OR REPLACE TABLE `__PROJECT__.retail_star.dim_customer` (
  customer_key INT64,
  customer_id STRING,
  full_name STRING,
  gender STRING,
...
INSERT INTO `__PROJECT__.retail_star.dim_customer` VALUES
...
CREATE OR REPLACE TABLE `__PROJECT__.retail_star.dim_store` (
  store_key INT64,
  store_id STRING,
  store_name STRING,
  city STRING,
...
INSERT INTO `__PROJECT__.retail_star.dim_store` VALUES
...
CREATE OR REPLACE TABLE `__PROJECT__.retail_star.dim_channel` (
  channel_key INT64,
  channel_name STRING,
  channel_group STRING
);

INSERT INTO `__PROJECT__.retail_star.dim_channel` VALUES
(1,'In-Store','Offline'),
(2,'Website','Online'),
(3,'Mobile App','Online'),
(4,'Marketplace','Online');

CREATE OR REPLACE TABLE `__PROJECT__.retail_star.dim_date` AS
WITH dates AS (
...
CREATE OR REPLACE TABLE `__PROJECT__.retail_star.fact_sales` (
  sales_key INT64,
  order_id STRING,
  order_line_id INT64,
  date_key INT64,
...
INSERT INTO `__PROJECT__.retail_star.fact_sales`
WITH base AS (
  SELECT
    txn AS sales_key,
...
joined AS (
  ...
  FROM base b
  JOIN `__PROJECT__.retail_star.dim_product` p
